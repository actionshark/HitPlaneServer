...
public class Server implements ServerEndpoint {
	...
	public synchronized void onLogin(UserInfo ui, UserInfo old) {
		if (old == null) {
			ui.onLogin();
		} else {
			mUsers.remove(old.session);
			old.session = ui.session;
			mUsers.put(ui.session, old);
			
			old.markRequest();
			
			if (old.status == UserInfo.STATUS_OFFLINE) {
				old.status = UserInfo.STATUS_IDLE;
			}
		}
	}
